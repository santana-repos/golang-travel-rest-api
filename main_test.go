...
func TestHandler(t *testing.T) {
	...
	req, err := http.NewRequest("GET", "/cli/GetMinorPriceRoute?name=Santana", nil)
...
func TestCallServer(t *testing.T) {
	resp, err := http.Get("http://localhost:8080/cli/GetMinorPriceRoute?name=Santana")
	if err != nil {
		t.Errorf("got error to prepare request: %v", err)
	}
	...
	if err != nil {
		t.Errorf("got error read response: %v", err)
	}
